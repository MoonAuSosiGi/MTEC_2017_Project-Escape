Shader "UBER - DynSnow - Metallic Setup/(RTP's Geom Blend)/Triplanar Selective - Two Layers"
{
	Properties
	{
		...
		[Foldout] _GeomBlendShown ("", Float) = 1				
	...
	CGINCLUDE
		...
		//#define TRANSLUCENCY_VERTEX_COLOR_CHANNEL b

		// if defined we can control occlusion by vertex color
		//#define OCCLUSION_VERTEX_COLOR_CHANNEL b
		...
		// make it geom blend for RTP
		// coverage channel is like in RTP - vertex color alpha
		// look that variants for transparency are removed and shader is ALWAYS "transparent fade"
		// we need also "GEOM_BLEND"="On" tag enabled for inspector
		#define GEOM_BLEND
	...
	SubShader
	{
		Tags { "RenderType"="Opaque" "Queue"="Geometry+12" "PerformanceChecks"="False" "SNOW"="On" "REFR"="Off" "TWO_LAYERS"="On" "TRIPLANAR"="On" "GEOM_BLEND"="On"}
		LOD 700
		Offset -1,-1
		ZTest LEqual	
...
//		*/			
		...
		Pass
		{
			...
			#define _ALPHABLEND_ON 1 // GEOM BLEND - always transparent
...
//			//#pragma shader_feature _CHROMATIC_ABERRATION
		...
		Pass
		{
			...
			//Blend [_SrcBlend] One
			ColorMask RGB Blend SrcAlpha One	
